...
function AccountChart({ transactions }) {
    ...
    const filterData = useMemo(() => {
        ...
        const now = new Date();
        const startDate = range.days ? startOfDay(subDays(now, range.days)) : startOfDay(newDate(0));

        const filtered = transactions.filter((t) => new Date(t.date) >= startDate && new Date(t.date) <= endOfDay(now));

        const grouped = filtered.reduce((acc, transaction) => {
            const date = format(new Date(transaction.date), "MMMM dd")
    ...
    const totals = useMemo(() => {
        return filterData.reduce((acc, day) => ({
    ...
    return (
        ...
        <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-7">
                <CardTitle className="text-semibold font-normal">Transaction Overview</CardTitle>
